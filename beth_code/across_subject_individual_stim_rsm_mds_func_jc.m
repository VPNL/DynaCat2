% DynaCat + Staticat: Functional ROI RSMs, individual stimuli
%   -Computes multivoxel and plots RSMs using indiviudal stimuli as conditions 
%       for each subject and then across subjects within functional ROIs.
...
for r=1:length(ROIs) % loop over ROI
    ROIname = ROIs{r};
    subs_w_roi = [];
    ...
    for s=1:length(subjects)
        ...
        if exist(ROI_filepath, 'file') % ROI exists
            subs_w_roi = [subs_w_roi; s];
            if isempty(seen_by_any)
                seen_by_any = seen_conds;
            else
                seen_by_any = union(seen_by_any, seen_conds);
            end

            %vANATOMYPATH = fullfile(session_path,'3DAnatomy/t1.nii.gz'); % Set vAnatomy
            ...
            resultsDir = fullfile(session_path, 'results', 'func_rois_rsms');
            if ~exist(resultsDir,'dir')
                mkdir(resultsDir);
            end
            rsm_filepath = fullfile(resultsDir, [ROIname '_individual_stim_mean_rsm.mat']);
            save(rsm_filepath, 'rsm')
            % save subject all cond rsm data
            rsm_filepath = fullfile(resultsDir, [ROIname '_individual_stim_all_conds_mean_rsm.mat']);
            save(rsm_filepath, 'all_cond_rsm')
            % save mv data struc
            mv_filepath = fullfile(resultsDir, [ROIname '_mv.mat']);
            save(mv_filepath, 'mv')
    ...
    % seen conditions across all subjects
    n_subj_rois = size(subs_w_roi, 1);
    seen_by_any = sort(seen_by_any);
    seen_by_any = seen_by_any(2:end - 1); % remove baseline + repeat

    % get mean across subjects rsms per roi
    n_subj_rois = size(subs_w_roi, 1);
    across_subject_rsm = across_subject_rsm(:,:,subs_w_roi); % remove subjects without the roi
    mean_rsm = mean(across_subject_rsm, 3, 'omitnan');
    mean_rsm = mean_rsm(seen_by_any, seen_by_any);

    % plot across-subject rsm
    figure('color', [1 1 1], 'name', [ROIname ' across-subjects (n=' num2str(n_subj_rois) ') ' plot_type ': ' ROIname],'units','norm', 'position', [ 0.1 .1 .6 .6]);
    imagesc(mean_rsm, [-.7 .7]); axis('image');
    cmap=mrvColorMaps('coolhot'); colormap(cmap);
    hcb=colorbar; title(hcb, 'correlation');

    plot_title = [ROIname ' across-subjects rsm (n=' num2str(n_subj_rois) ')'];
    ...
    set(gca, 'clim', [-0.35, 0.35]); % clim([-0.35, 0.35]);
    brighten(0.6)

    % save figure
    OutDir = fullfile(ExpDir ,'results', 'func_rois', 'rsm_mds');
    if ~exist(OutDir,'dir')
        mkdir(OutDir);
    end
    ...
    outfile=fullfile(PngDir , RSM_filename_png);
    saveas(gcf, outfile, 'png')

    SvgDir = fullfile(OutDir, 'svg');
    if ~exist(SvgDir,'dir')
        mkdir(SvgDir);
    end
    RSM_filename_svg=[ROIname '_rsm_across-subjects_individual_stim.svg'];
    outfile=fullfile(SvgDir , RSM_filename_svg);
    ...
    outfile=fullfile(FigDir , RSM_filename_fig);
    savefig(gcf, outfile, 'fig');
    
    % save across subject rsm data
    across_subject_rsm_filepath = fullfile(OutDir, [ROIname '_across_subject_rsm_individual_stim.mat']);
    save(across_subject_rsm_filepath, 'across_subject_rsm')
    % save mean rsm data
    mean_rsm_filepath = fullfile(OutDir, [ROIname '_mean_rsm_individual_stim.mat']);
    save(mean_rsm_filepath, 'mean_rsm')

    close all

    % get mean MDS across-subjects per roi
    group_distance_matrix = group_distance_matrix(:,:,subs_w_roi); % remove subjects without the roi
    mean_distance_matrix = mean(group_distance_matrix, 3, 'omitnan');
    mean_distance_matrix = mean_distance_matrix(seen_by_any, seen_by_any); % remove unseen conditions
    cond_labels = all_cond_labels(seen_by_any); % apply to relevant all_cond vars
    cond_point_shapes = all_cond_point_shapes(seen_by_any);
    cond_label_colors = all_cond_label_colors(seen_by_any);

    [r_idx, c_idx] = find(isnan(mean_distance_matrix)); % find conditions without a comparison
    r_idx = sort(unique(r_idx));
    mean_distance_matrix(r_idx,:) = [];
    mean_distance_matrix(:,r_idx) = [];
    cond_labels(r_idx) = []; % apply to relevant all_cond vars
    cond_point_shapes(r_idx) = [];
    cond_label_colors(c_idx) = [];
    ...
    for i = 1:numel(cond_labels)
        ...
        [stim_icon, map, alphachannel] = imread(fullfile(stimuli_icon_folder, [cond_labels{i} '.png']));
    ...
    plot_title = [ROIname ' across-subjects individual stim mds n=(' num2str(n_subj_rois) ')'];
    title(plot_title, 'Interpreter','none', 'FontSize',10);
    ylabel('axis 1'); xlabel('axis 2');

    % save figure
    OutDir = fullfile(ExpDir ,'results', 'func_rois');
    ImageDir = fullfile(OutDir, 'images');
    % as png
    filename_png=[ROIname '_individual_stim_mds' stimuli_icon_label '.png'];
    outfile=fullfile(PngDir , filename_png);
    saveas(gcf, outfile, 'png')
    saveas(gcf, fullfile(ImageDir, filename_png), 'png')
    % as svg
    filename_svg=[ROIname '_individual_stim_mds' stimuli_icon_label '.svg'];
    outfile=fullfile(SvgDir , filename_svg);
    saveas(gcf, outfile, 'svg')
    saveas(gcf, fullfile(OutDir, filename_svg), 'svg')
    % as figure
    filename_fig=[ROIname '_individual_stim_mds' stimuli_icon_label '.fig'];
    outfile=fullfile(FigDir , filename_fig);
    savefig(gcf, outfile)
    savefig(gcf, fullfile(OutDir, filename_fig))
    ...
    for i = 1:numel(cond_labels)
        if i < 193
            scatter(Y(i,1), Y(i,2), categeory_color_point_size_roi, 'Marker', cond_point_shapes{i}, 'MarkerFaceColor', cond_label_colors{i}, 'MarkerEdgeColor', cond_label_colors{i});
        else
            scatter(Y(i,1), Y(i,2), categeory_color_point_size_roi, 'Marker', cond_point_shapes{i}, 'MarkerEdgeColor', cond_label_colors{i});
        end
        hold on;
    end
    ...
    plot_title = [ROIname ' individual stim mds category colors n=(' num2str(n_subj_rois) ')'];
    title(plot_title, 'Interpreter','none', 'FontSize',10);
    ylabel('axis 3'); xlabel('axis 4');
